...
val BINARY_OPERATIONS = arrayOf<BinaryOperation>(
    ...
    object: BinaryOperation('≥', -1){
        override fun compute(a: Double, b: Double): Double = if(abs(a - b) <= EPS || a > b) 1.0 else 0.0
    },
    ...
    object: BinaryOperation('&', -1){
        override fun compute(a: Double, b: Double): Double = if(abs(a) >= EPS && abs(b) >= EPS) 1.0 else 0.0
    },
